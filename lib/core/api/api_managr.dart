...
class ApiManager {
  ...
  Future<AuthLoginModel> login(RequestLogin requestLogin) async {
    ...
    try {
      ...
      ResponseErrorModel errorModel = ResponseErrorModel(codigoError: e.response != null ? e.response!.statusCode! : 0);
      return Future.error(errorModel);
    }
  }


  Future<ValidationQrResponse> validateQrCode(ValidateQrCodeRequest validateQrCodeRequest) async {
    response_dio.Response response;
    try {
      ...
      if (response.data['mensaje'] != null && response.data['mensaje']['status'] == 'A' ) {
  ...
  Future<bool> RetirarEstudiante(RetirarRequest retirarRequest) async {
    ...
    try {
      ...
      response = await _dio.post('/retirar',
        data: retirarRequest.toJson(),
        options: opcionesHeader()
      );
      print('response retirar' + response.data.toString());
      if (response.data['status'] != null && response.data['status'] == 'Operación realizada OK') {
        return true;
      } else {
        return true;
      }
  ...
  Future<List<ResponseSalida>> getPadresSalida(SalidaRequest salidarequest) async {
    response_dio.Response response;
    try {
      response = await _dio.post('/salida',
        data: salidarequest.toJson(),
        options: opcionesHeader()
      );
  ...
  Future<String> marcarSalida(MarcarSalidaRequest marcarSalidaRequest) async {
    ...
    try {
      response = await _dio.post('/marcar_salida',
        data: marcarSalidaRequest.toJson(),
        options: opcionesHeader()
      );
